...
/**
 * Sign a payload with HMAC-SHA256 using the admin password as key.
 */
...
/**
 * Verify an HMAC-signed payload. Returns the original payload if valid, null otherwise.
 */
async function verify(signed: string): Promise<string | null> {
  const secret = getSecret();
  if (!secret) return signed; // no password configured = no auth
